  ...
  return (
    <div className="flex items-center gap-1.5">
      <button
        ...
        title="Liked it"
        className={`${btnSize} cursor-pointer rounded transition-colors disabled:opacity-50 ${
          currentRating === 1
            ? 'text-trove-accent'
            : 'text-trove-muted hover:text-trove-accent'
        }`}
      >
        <svg className={iconSize} viewBox="0 0 24 24" fill={currentRating === 1 ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth={2}>
      ...
      <button
        ...
        className={`${btnSize} cursor-pointer rounded transition-colors disabled:opacity-50 ${
          currentRating === -1
            ? 'text-red-400'
            : 'text-trove-muted hover:text-red-400'
        }`}
      >
        <svg className={iconSize} viewBox="0 0 24 24" fill={currentRating === -1 ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth={2}>
